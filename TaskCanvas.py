...
def _resolve_initial_projects(tasks_all: list[dict], args_wo_filter: list[str]) -> list[str]:
    init_projects = []
    if any(arg == "--selector" for arg in args_wo_filter):
        try:
            init_projects = run_project_selector(tasks_all)
        except Exception as e:
            print(f"[selector] error: {e}")

    extra = [arg for arg in args_wo_filter if arg and not arg.startswith("-")]
...
def _apply_background(html: str, args_wo_filter: list[str]) -> str:
    bg_arg, bg_opacity, _ = _extract_bg_args(args_wo_filter)
...
def main() -> int:
    ...
    try:
        init_task_uuids = _resolve_initial_task_uuids(filter_str)
    except ValueError as e:
        return _fail(
            str(e),
            tip="Tip: quote filter expressions, e.g. --filter 'project:Work +P1'",
    ...
    init_projects = _resolve_initial_projects(tasks_all, args_wo_filter)
    ...
    try:
        html = _apply_background(html, args_wo_filter)
    except ValueError as e:
        return _fail(
            str(e),
