...
from kyanbasu.runtime_support import (
    inject_body,
    inject_body_once,
    inject_head,
    inject_head_once,
...
def _find_bg_file(prefer: str | None, base_dir: Path | None = None, include_demo: bool = False):
    ...
    cwd = Path.cwd()
    repo_root = script_dir.parent if script_dir.name == "kyanbasu" else script_dir
    ...
    names = ["kyanbasu-bg", "Kyanbasu.bg", "canvas-bg", "background", "bg"]
...
def inject_runtime_diagnostics(html: str) -> str:
    ...
    Lightweight browser diagnostics:
      ...
      - exposes snapshot via window.KyanbasuDiagnostics()
    """
